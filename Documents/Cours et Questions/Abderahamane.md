...
Par contre les fichiers que vous aurez crée dans des applications présentes sur votre machine (word, excel, open office...etc.), vous voudrez probablement les organiser dans des endroits précis.  
...
- Si vous avez bien fait les choses, vous avez un dossier d'enregistrement par application, et dans ce cas facile, vous allez dedans et retrouvez votre bazarre.
...
Et une mauvaise manipulation peut vous mettre dans l'embaras, meaning envoyer l'ordinateur au SAV ^^.  
...
Tant de paramètres, qui quand vous lancez votre machine pour la première fois vous font dire : "Bizarre ça, pourquoi mon ordinateur à 4000 euros ouvre l'explorateur de fichier encore plus lentement que l'ordinateur du CDI ?".  
...
Et de savoir quand un composant commence à devenir vraiment "outdated", obsolète en bon français.  
...
Il en existe plein d'autre type pour des usages précis :  
...
- La partie Mémoire : vérifiez absolument que le "profil XMP" est activé ! Par défaut, les barettes de DRAM sont paramètré pour fonctionner à leur fréquence minimale. On est heureusement plus à l'époque de l'esclavage ou pour accélérer la cadence il fallait sortir le fouet (quoi que vous me direz, maintenant c'est plus le fouet, c'est le salaire, mais ne parlons pas de sujet qui fache ^^), il faut donc activer le profil XMP (eXtreme Memory Profile pour les CPU Intel) ou EXPO (si vous avez un CPU AMD) dans le BIOS.  
...
Vous trouverez une ribanbelle de tutoriel et cours qui vous présonteront en détails l'étendu des commandes windows existantes.
...
rd /q [nom du dossier]  
Supprime le dossier sans demander de confirmation.

rd /s /q [nom du dossier]  
Supprime le dossier et tout les fichiers qu'il contient sans demander de confirmation.
